    ...
    let currentPage = 1;
    let itemsPerPage = 12;
    let currentFilteredProducts = [...(typeof productos !== 'undefined' ? productos : [])];
    ...
    function renderCatalogoConPaginado() {
      const startIndex = (currentPage - 1) * itemsPerPage;
      const endIndex = startIndex + itemsPerPage;
      const productosPagina = currentFilteredProducts.slice(startIndex, endIndex);
      ...
      if (productosPagina.length === 0 && currentFilteredProducts.length === 0) {
    ...
    function aplicarFiltros() {
      const categoriaActiva = document.querySelector('.category-btn.active')?.dataset.cat || 'todos';
      const tipoSeleccionado = document.querySelector('[name="tipo-filtro"]:checked')?.value || 'todos';
      
      let productosFiltrados = [...(typeof productos !== 'undefined' ? productos : [])];
      ...
      currentFilteredProducts = productosFiltrados;
      currentPage = 1;
      renderCatalogoConPaginado();
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (searchQuery) {
        ...
        if (input) {
          ...
          if (resultados.length > 0) {
            ...
            currentFilteredProducts = (typeof productos !== 'undefined' ? productos : []).filter(p => ids.includes(p.id));
          } else {
      ...
      currentFilteredProducts = [...(typeof productos !== 'undefined' ? productos : [])];
      renderCatalogoConPaginado();
      
      document.getElementById('prev-page-btn')?.addEventListener('click', () => { if (currentPage > 1) { currentPage--; renderCatalogoConPaginado(); } });
      document.getElementById('next-page-btn')?.addEventListener('click', () => { const total = Math.ceil(currentFilteredProducts.length / itemsPerPage); if (currentPage < total) { currentPage++; renderCatalogoConPaginado(); } });
      document.getElementById('page-input')?.addEventListener('change', function() { let np = parseInt(this.value); const total = Math.ceil(currentFilteredProducts.length / itemsPerPage); if (isNaN(np)) np = 1; if (np < 1) np = 1; if (np > total && total > 0) np = total; currentPage = np; renderCatalogoConPaginado(); });
    });
