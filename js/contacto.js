    ...
    function renderSearchResults(resultados, container) {
      ...
      container.innerHTML = resultados.map(p => `
        <div class="search-result-item" onclick="window.location.href='detalles.html?id=${p.id}'">
          <img src="${p.img}" onerror="this.src='https://picsum.photos/seed/${p.id}/50/50'" alt="${p.nombre}">
    ...
    function loadCart() {
      ...
      if (saved) {
        try { carrito = JSON.parse(saved); } catch(e) { carrito = []; }
      }
      updateCartUI();
    }
      ...
      if (carrito.length === 0) {
        body.innerHTML = `
          <div class="cart-empty">
            ...
            <p style="font-size: 0.85rem; margin-top: 0.3rem;">Explora nuestro catálogo y añade tus piezas favoritas</p>
          </div>
        `;
      ...
      body.innerHTML = carrito.map(item => `
        <div class="cart-item">
          <div class="cart-item-info">
            ...
            <div class="cart-item-meta">${item.precio} c/u</div>
          </div>
    ...
    function initCartUI() {
      ...
      document.getElementById('cartGoToBtn').addEventListener('click', () => {
        toggleCart();
    ...
    document.addEventListener('DOMContentLoaded', function() {
      if (typeof productos === 'undefined') {
        console.warn('La variable productos no está definida.');
      }
