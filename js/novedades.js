    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const nuevos = [...(typeof productos !== 'undefined' ? productos : [])].sort((a, b) => b.id - a.id).slice(0, 4);
      const populares = typeof productos !== 'undefined' ? productos.filter(p => p.destacado === true) : [];
      const nuevosGrid = document.getElementById('nuevos-grid');
      const popularesGrid = document.getElementById('populares-grid');
      if (nuevosGrid) { nuevosGrid.innerHTML = ''; nuevos.forEach(p => nuevosGrid.appendChild(createProductCard(p))); }
      if (popularesGrid) { popularesGrid.innerHTML = ''; populares.forEach(p => popularesGrid.appendChild(createProductCard(p))); }
    });
